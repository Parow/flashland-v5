...
RegisterNUICallback('setTorch', function (data, cb)
  torch = data
  cb()
end)

RegisterNUICallback('setPlanes', function (data, cb)
  planes = data
  cb()
end)
...
--         camcoords = GetEntityCoords(ped)
--         x,y,z = table.unpack(camcoords)
--         z = z + 1.2
--         camcoords = vector3(x,y,z)
--         local forward = GetEntityForwardVector(ped)
--         local forward = GetGameplayCamRot()
--         DrawSpotLightWithShadow(camcoords, forward, 255, 255, 255, 80.0, 1.0, 12.0,6.0, 200.0, 10.0)
--      --   DrawSpotLight(, forward, 255, 255, 255, 80.0, 10.0, 12.0, 6.0, 200.0, 10.0)
--       end
--   end
-- end)
...
--       local playerPed = PlayerPedId()
--       local coords    = GetEntityCoords(playerPed)
--       x1,y1,z1        = table.unpack(coords)
--       z1 = z1+1.2
--       local forward   = GetEntityForwardVector(playerPed)
--       local x, y, z   = table.unpack(coords - forward)
--       DrawSpotLightWithShadow(x1,y1,z1,  x, y, z, 255, 255, 90, 80.0, 10.0, 12.0, 6.0, 200.0, 10.0)
--   end
-- end)
...
Citizen.CreateThread(function()
  while true do
      Wait(0)
      if torch then
        AdvanceClockTimeTo(5.0,5.0,5.0)
        local ped = GetPlayerPed(-1)
        camcoords = GetEntityCoords(ped)
        x,y,z = table.unpack(camcoords)
        z = z + 1.2
        camcoords = vector3(x,y,z)
        local forward = GetEntityForwardVector(ped)
        DrawSpotLight(camcoords, forward, 255, 255, 90, 80.0, 10.0, 12.0, 6.0, 200.0, 10.0)
      end
  end
end)
